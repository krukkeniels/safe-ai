...
| Workspace & context isolation to /workspace | R6 | Named volume mount; sandbox cannot access host filesystem |
...
|---------|-------------|-------------------------------|
| Action approval & output control for destructive actions | R4 | safe-ai ships a git pre-push hook, but cannot gate arbitrary agent tool use (file deletion, API calls, CI modifications). Approval gates must be implemented at the agent platform level. |
...
These actions are mitigated by code review (R9), action approval & output control (R4), and audit logging (R5) -- not by the sandbox itself.
